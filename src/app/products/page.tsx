    ...
    <main className="w-full min-h-screen bg-[#050505] flex flex-col">
      ...
      <section className="relative w-full h-[60vh] md:h-[80vh] flex items-center justify-center border-b border-white/5 overflow-hidden">
        <div className="absolute inset-0 z-0 bg-[#020202]">
          <Canvas camera={{ position: [0, 5, 10], fov: 50 }} dpr={[1, 1.5]}>
            <Suspense fallback={null}>
              ...
              <directionalLight position={[10, 10, 5]} intensity={2} color="#ff4500" />
        ...
        <div className="relative z-10 text-center container mx-auto px-6">
          <h4 className="text-[#4a50c8] font-bold tracking-widest text-sm mb-4 uppercase">Our Systems</h4>
        ...
        <div className="container mx-auto px-6 max-w-5xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            {[
              ...
              <div key={i} className="p-6 border border-white/10 bg-white/5 rounded-sm hover:border-[#ff4500] transition-colors group cursor-pointer">
                <span className="text-[#ff4500] font-mono text-xs mb-2 block">{String(i+1).padStart(2, '0')}</span>
                <h3 className="text-white font-bold text-xl group-hover:text-[#ff4500] transition-colors">{prod}</h3>
              </div>
            ))}
          </div>
        </div>
