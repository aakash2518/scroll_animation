    ...
    <section ref={sectionRef} id="infrastructure" className="relative h-[80vh] min-h-[600px] overflow-hidden bg-[#050505] flex items-center border-t border-white/5">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="parallax-bg absolute -inset-[20%] bg-[#0a0a0a] z-0">
          ...
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#0077B6] rounded-full mix-blend-screen filter blur-[150px] opacity-20"></div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            {facilities.map((fac, i) => (
              <div key={i} className="glass-panel p-6 md:p-8 border border-white/10 rounded-sm hover:border-[#0077B6]/50 transition-colors duration-300 group cursor-default relative overflow-hidden backdrop-blur-xl">
                <div className="absolute inset-0 bg-[#0077B6]/10 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out"></div>
                <span className="text-[#0077B6] font-mono text-xs tracking-widest mb-4 block relative z-10">{fac.id}</span>
