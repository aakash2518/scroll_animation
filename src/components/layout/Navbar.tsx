    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#050505]/95 backdrop-blur-sm border-b border-white/10 py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                key={link.name} 
                href={link.href} 
                className={`text-xs font-bold tracking-widest transition-colors ${isActive ? 'text-[#4a50c8]' : 'text-white hover:text-[#4a50c8]'}`}
              >
                {link.name}
              </Link>
            );
        ...
        <div className="hidden md:block">
          <Link href="/contact-us" className="px-6 py-3 bg-[#4a50c8] hover:bg-[#3d44b0] transition-all text-xs font-bold tracking-widest rounded-sm text-white flex items-center gap-2 group shadow-lg shadow-[#4a50c8]/20">
            GET A QUOTE
